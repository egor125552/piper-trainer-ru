...
## Прогресс в работающей ячейке Colab

После запуска первых пяти ячеек последняя ячейка не завершается: она
показывает ссылку на Gradio и далее остаётся выполняться. Интерфейс Gradio
не встраивается в вывод ячейки. Кнопки распознавания и обучения работают
в браузере как раньше, а их прогресс печатается прямо под пятой ячейкой.

Qwen показывает «расшифровано N/M; осталось K» после каждого блока и
прогресс привязки слов. Piper показывает начало и конец каждой эпохи,
номер шага и сохранение recovery. Служебная полоса прогресса отключена.

Сообщения из обработчиков веб-интерфейса передаются в локальный журнал,
который работающая ячейка проверяет каждые полсекунды. Чтобы остановить
монитор, вручную прервите пятую ячейку. Не нужно создавать дополнительные
ячейки запуска ASR или обучения, и существующие датасеты не меняются.
